...
from src.cat_scan_filters.frangi import save_frangi
...
parser.add_argument(
    ...
    default=os.path.join(WORKDIR, "CAD_PE/images"),
)
parser.add_argument(
    "--output_dir",
    help="Directory where the output files are to be written",
    default=os.path.join(WORKDIR, "CAD_PE/frangi"),
)
parser.add_argument("--input_regex", help="regex to identify input files", default="")
parser.add_argument(
    "--remove_suffix",
    help="Suffix that should me removed from the input file's name in order to produce the frangi file's name",
    default="",
)
parser.add_argument(
    "--suffix",
    help="The suffix that will be added to the input file's name in order to produce the frangi file's name",
    default="_frangi",
)
parser.add_argument(
    "--force",
    help="Forces the frangi vesselness to be recomputed if a file already exists",
    action="store_true",
)
